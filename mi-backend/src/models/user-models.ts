...
export class UserModel {
  ...
  static validateEmail(email: string): { valid: boolean; message?: string } {
    ...
    if (!this.allowedDomains.includes(domain))
      return {
        valid: false,
        message: `Dominio no permitido. Solo se aceptan: ${this.allowedDomains.join(', ')}`,
      };
  ...
  static validatePhone(phone: string): { valid: boolean; message?: string } {
    ...
    const re = /^[0-9]{10,15}$/;
    if (!re.test(phone))
      return { valid: false, message: 'Teléfono inválido. Debe tener entre 10 y 15 dígitos' };
    return { valid: true };
  }
  ...
  static validateToken(token: string): { valid: boolean; message?: string } {
    if (!token || token.length <= 10)
      return { valid: false, message: 'Token inválido o muy corto' };
    return { valid: true };
  }

  static validateCoordinates(lat: number, lng: number): { valid: boolean; message?: string } {
    if (typeof lat !== 'number' || typeof lng !== 'number')
      return { valid: false, message: 'Coordenadas deben ser números' };
    if (lat < -90 || lat > 90 || lng < -180 || lng > 180)
      return { valid: false, message: 'Coordenadas fuera de rango' };
    return { valid: true };
  }
